...
/** @file
 *  @brief BabbleSim central role for the gatt_gen_secure self-test.
 *
...
static void test_central(void)
{
	...
	uint16_t secure_svc_secret_handle = discover_chrc(SECURE_SVC_SECRET_UUID);
	if (secure_svc_secret_handle == 0) {
		TEST_FAIL("secure_svc/secret not found");
	}
	...
	TEST_PASS("gatt_gen_secure central passed");
}
